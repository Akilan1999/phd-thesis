...
based in \ref{tab:unikerneltable}. Based on the number of targets supported Unikraft has the most amount of 
targets supported. Since the the research goals (//todo refer research goals) for using Uni-kernels is to run on 
bare-metal as a major requirement (This is because of the way multi-kernels work(//todo refer to the multi-kernel
section once complete)).Unikraft would be suitable for testing a multi-kernel environment, but porting to 
...
Unikraft does not currently support Multi-core mode yet. By default it uses the library
...
OSv supports running application in multiple cores. OSv thread scheduler is lock-free, 
...
\subsubsection{Timder V \cite{weiser_timber-v_2019}}
...
\subsubsection{ARM MTE \cite{ARMMTE}}
...
\subsubsection{D-RI5CY \cite{D-RISCY}}
...
\subsubsection{HyperFlow \cite{HyperFlow}} 
...
\subsubsection{Typed Architecture \cite{TypedArchitecture}}
...
\subsubsection{Dover \cite{Dover}}
...
The following paper focuses on an architectural hardware bounded pointer primitive 
