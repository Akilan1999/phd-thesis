...
implementation of a Uni-kernel currently maintained. This question will be answered 
...
popcorn linux \cite{PopcornEnclave} which allows programmers to annotate which parts of their program who they want to offload and run it 
IntelSGX as an example. The HEXO can be considered as a paper which uses the Multi-kernel implementation in combination with Uni-kernels
...
and the Cross-ISA enclave offloading using popcorn linux utilizes the IntelSGX\cite{IntelSGX}(Enclave model\cite{Enclave}) can considered as 
security enhancement to Multi-kernel model. On the perspective of security IntelSGX cannot be considered as 
a TAG based because it does not tie tags with addresses and only covers a sub-set of security policies in contract 
...
Popcorn linux would significantly change based on the possibility of adding TAG-based as a factor into the 
Popcorn linux heterogeneous variety of architectures and based on Survey conducted there has been 1 paper\cite{TypedArchitecture} 
which talks about utilizing TAG based-hardware to speedup dynamic checking. This opens the possibility to explore designing a 
